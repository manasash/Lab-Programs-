...
cannot go beyond Vmin, But
%quantiz may suggest it, since it return the Values lower than Actual Signal Value
subplot(3,1,3);
stem(q);
...
q = q + (Vmin+(StepSize/2));
% Above step gives a DC shifted version of Actual signal, Thus it is necessary to
bring it to zero level
subplot(2,1,2);
grid on;
